...
def precompute_freqs_cis(dim: int, end: int = int(32 * 1024), rope_base: float = 1e6):
    ...
    """
    # 计算频率：θ_i = base^(-2i/d), i ∈ [0, d/2)，生成 dim//2 个频率：步长为2，范围为0~dim(hidden_size)//2
    freqs = 1.0 / (rope_base ** (torch.arange(0, dim, 2).float() / dim))
    
    # 计算每个位置的频率：pos * θ_i，形状 (end, dim//2)，outer是矩阵乘法
    freqs = torch.outer(torch.arange(end, device=freqs.device), freqs).float()
    
    # 计算 cos 和 sin，并复制一次以匹配 head_dim（用于两两分组）
    freqs_cos = torch.cat([torch.cos(freqs), torch.cos(freqs)], dim=-1)  # (end, dim)，cat拼接，-1是在最后一维拼接
...
class Attention(nn.Module):
    ...
    def __init__(self, args: SpongeBobConfig):
        ...
        self.head_dim = args.hidden_size // args.num_attention_heads #单个头的长度
    ...
    def forward(self,
        ...
        if past_key_value is not None:
            xk = torch.cat([past_key_value[0], xk], dim=1) # 将past_key_value[0]和xk拼接起来，dim=1是在第二维拼接
            xv = torch.cat([past_key_value[1], xv], dim=1) # 将past_key_value[1]和xv拼接起来，dim=1是在第二维拼接
        ...
        # 恢复形状并输出投影，output:(batch, num_heads, seq_len, head_dim) -> (batch, seq_len, num_heads * head_dim)
        output = output.transpose(1, 2).reshape(bsz, seq_len, -1)
        output = self.resid_dropout(self.o_proj(output))
        return output, past_kv

# “我创建了一个名为 FeedForward 的零件（继承自 nn.Module），为了组装它，我塞给它一本说明书（参数 config）。”
# self 是指当前这个类的“实例（Instance）”，而不是类（Class）本身。
...
class SpongeBobBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: SpongeBobConfig):
        ...
        self.layer_id = layer_id #每个block的K，V均不一样，需要用layer_id来区分
        self.input_layernorm = RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = RMSNorm(config.hidden_size, eps=config.rms_norm_eps) # RMSNorm有参数需要学习，所以需要定义两个RMSNorm
    ...
    def forward(self, hidden_states, position_embeddings, past_key_value=None, use_cache=False, attention_mask=None):
        ...
        residual = hidden_states # 输入会变化，需要保存副本
...
class SpongeBobModel(nn.Module):
    ...
    def __init__(self, config: SpongeBobConfig):
        ...
        # Token Embedding
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size) # 本质就是一个线性层，但由于独热编码性质，可直接查表，速度更快
        self.dropout = nn.Dropout(config.dropout)
        
        # Transformer Blocks，l是layer_id，用于区分每个block的K，V均不一样
        ...
        # 不在block中计算，为了防止反复计算频率
        freqs_cos, freqs_sin = precompute_freqs_cis(
            dim=config.hidden_size // config.num_attention_heads,
            end=config.max_position_embeddings,
            rope_base=config.rope_theta
        )
        self.register_buffer("freqs_cos", freqs_cos, persistent=False) # 注册为buffer，不参与训练但会保存在模型中
        self.register_buffer("freqs_sin", freqs_sin, persistent=False)

    def forward(self,
                input_ids: Optional[torch.Tensor] = None,  # 我希望 input_ids 是个张量或空，而且如果你调用我不传这个参数，我就默认它是 None
                attention_mask: Optional[torch.Tensor] = None,
        ...
        # 处理 past_key_values
        if hasattr(past_key_values, 'layers'):  # 如果是第一个token，past_key_values是没有的，所以为None
            past_key_values = None
        past_key_values = past_key_values or [None] * len(self.layers) # 如果past_key_values为None，则用None填充
        
        # 计算起始位置（用于 RoPE），past_key_values[0][0]是第一层的K，shape[1]是第二个维度，即seq_len
...
class SpongeBobForCausalLM(PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: SpongeBobConfig = None):
        ...
        self.model.embed_tokens.weight = self.lm_head.weight  # 权重绑定，输入和输出两个头共用一个权重矩阵，减少参数数量
    ...
    def forward(self,
        ...
        logits = self.lm_head(hidden_states[:, slice_indices, :]) # 只保留最后几个token的logits
        ...
        if labels is not None:
            ...
            shift_labels = labels[..., 1:].contiguous() # 我们要预测的是下一个token，所以labels需要右移一位刚好对应的下一个token
            loss = F.cross_entropy(
                shift_logits.view(-1, shift_logits.size(-1)), 
                shift_labels.view(-1), 
                ignore_index=-100  # 忽略 padding 和 mask 的位置，-100是label的padding值
            )
